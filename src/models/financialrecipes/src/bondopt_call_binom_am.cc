...
double bond_option_price_call_american_binomial( const double& B,     // Bond price
   ...
   for (int i=0; i<=steps; ++i) put_values[i] = max(0.0, (prices[i]-K)); // put payoffs at maturity
   for (int step=steps-1; step>=0; --step) {
      for (int i=0; i<=step; ++i) {
	 put_values[i] = (p_up*put_values[i+1]+p_down*put_values[i])*Rinv;
	 prices[i] = d*prices[i+1];
	 put_values[i] = max(put_values[i],(prices[i]-K));       // check for exercise
      };
   };
   return put_values[0];
};
